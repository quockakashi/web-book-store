...
const getBookByIds = async (req, res, next) => {
    const { ids } = req.params;

    // Chuyển chuỗi ids thành mảng các ObjectId
    const idArray = ids.split('_').map(id => new ObjectId(id.trim()));

    try {
        let ratingFilter = 0;

        const searchCondition = { _id: { $in: idArray } }; // Tìm kiếm theo danh sách ID
